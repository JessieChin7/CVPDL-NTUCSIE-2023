...
def rescale_bboxes(out_bbox, size):
    ...
    scaling_tensor = torch.tensor([img_w, img_h, img_w, img_h], dtype=torch.float32).to(out_bbox.device)  # 移动张量到CUDA设备
    b = b * scaling_tensor
    return b
...
def main(args):
    ...
    if args.eval:
        ...
        if args.output_dir:
            utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval.pth")
        results = coco_evaluator.coco_eval["bbox"].eval
        log_stats = {**{f'test_{k}': v for k, v in test_stats.items()} }
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")

        # Load and process all images in the 'val' directory
        device = 'cuda'
